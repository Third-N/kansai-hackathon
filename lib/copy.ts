import type { Revision, SimResult, Spot } from "./types";
import { hhmm } from "./format";

/** 体力が尽きる区間のスポット名 */
export function collapseSpotName(
  result: SimResult,
  spots: Record<string, Spot>
): string | null {
  if (result.collapseMin === null) return null;
  const g = result.timeline.find(
    (s) => result.collapseMin! >= s.startMin && result.collapseMin! <= s.endMin
  );
  return g ? spots[g.spotId].name : null;
}

export interface InterruptCopy {
  title: string;
  body: string;
  primary: string;
  secondary: string;
...
export function interruptCopy(
  ...
  if (current.lowHpMin !== null || current.collapseMin !== null) {
    ...
    const segment = current.timeline.find(
      (s) => current.lowMpMin! >= s.startMin && current.lowMpMin! <= s.endMin
    );
  ...
  if (movedLater) {
    ...
    if (after) {
      ...
      if (cBefore !== null && cAfter !== null && cAfter < cBefore - 0.1) {
        parts.push(
          `${s.name}を${hhmm(after.startMin)}にずらすと、混雑が${Math.round(cBefore * 100)}%から${Math.round(cAfter * 100)}%まで下がります`
        );
      } else {

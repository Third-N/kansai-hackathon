import type { Revision, Round, RoundOption, RoundResult, Spot } from "./types";
...
export function vetoCap(optionCount: number): number {
  return Math.max(1, Math.floor(optionCount / 3));
}

/** 再構成案の並びを、投票にかけられる選択肢に変える */
export function optionsFromRevisions(
  revisions: Revision[],
...
function labelFor(r: Revision, spots: Record<string, Spot>): string {
  const { droppedId, insertedRestId, shortened, movedLater } = r.changes;
  if (droppedId) return `${spots[droppedId].name}をあきらめる`;
  if (insertedRestId && shortened) return `休憩を入れて、少しずつ短くする`;
  if (insertedRestId) return `${spots[insertedRestId].name}で休憩を挟む`;
  if (shortened) return `どこも少しずつ短くする`;
  if (movedLater) return `${spots[movedLater].name}を後ろにまわす`;
  return "順番を入れ替える";
}

function subFor(r: Revision): string {
...
export function resolveRound(
  options: RoundOption[],
  vetoes: { memberId: string; optionId: string }[],
  tiebreak: number
): RoundResult {
  ...
  const tally: VetoTally[] = options.map((o) => ({ optionId: o.id, count: counts.get(o.id)! }));
...
export function isRevealable(round: Round, now: number = Date.now()): boolean {

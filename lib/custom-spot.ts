...
const CATEGORY_SHAPE: Record<string, Record<number, number>> = {
...
const CATEGORY_WEIGHT: Record<string, { burn: number; joy: number; stayMin: number }> = {
 ...
 */
function fameAmplitude(priority: number): number {
  if (priority >= 1000) {
...
export function makeCustomSpot(input: {
  ...
  lat: number;
  lng: number;
  /** lib/local-place-search.ts の LocalPlaceCategory。無ければ「その他」扱い */
  category?: string;
  /** 端末内データのpriority。無ければ一般スポット並みの既定値 */
  priority?: number;
}): Spot {
  ...
  const crowdByHour = Object.fromEntries(
    Object.entries(shape).map(([hour, v]) => [Number(hour), Math.min(1, Math.round(v * amplitude * 100) / 100)])
  );

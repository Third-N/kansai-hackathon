...
for (const spot of Object.values(SPOTS)) {
  const scaled = (factor: number) => Object.fromEntries(
    Object.entries(spot.crowdByHour).map(([hour, value]) => [Number(hour), Math.min(1, value * factor)])
  );
  spot.crowdByDay = {
...
export const TRAVEL_TABLE: Record<string, number> = {
  "inari|kiyomizu": 30,
  "inari|nishiki": 25,
  "inari|nanzenji": 40,
  "kiyomizu|nishiki": 20,

...
describe("スキーマと権限", () => {
  ...
  it("あいことばの総当たりは、短時間に集中すると弾かれる", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      await b.raw.query("select join_by_code($1,'x',$2,12)", ["ハズレ" + i, uid(200 + i)] as never[]);
    }

    // 21回目以降は、正しいあいことばでも弾かれる
    let ok = true;
    try {
      await b.raw.query("select join_by_code($1,'あと',$2,12)", [code, uid(300)] as never[]);
    } catch { ok = false; }
    assert.equal(ok, false, "短時間の連続試行を絞り込めていない");
  });

  it("近くにいる人数は、同じ人が何度合図しても重複して数えない", async () => {
    await b.raw.query("select ping_spot('kiyomizu',$1)", [uid(1)] as never[]);
    await b.raw.query("select ping_spot('kiyomizu',$1)", [uid(1)] as never[]);
    await b.raw.query("select ping_spot('kiyomizu',$1)", [uid(2)] as never[]);
    await b.raw.query("select ping_spot('inari',$1)", [uid(3)] as never[]);

    const r = await b.raw.query<{ n: number }>("select nearby_crowd('kiyomizu') as n");
  ...
  it("参加者でなければ写真も足せない。大きすぎる写真は拒む", async () => {
    ...
    let ok = true;
    try {
      await b.raw.query("select add_photo($1,'inari','data:x',$2)", [tripId, uid(404)] as never[]);
    } catch { ok = false; }
    ...
    ok = true;
    try {
      await b.raw.query("select add_photo($1,'inari',$2,$3)", [tripId, tooBig, uid(1)] as never[]);
    } catch { ok = false; }

...
export function isLocalPlaceDataset(value) {
...
export function loadLocalPlaceDataset() {
    if (datasetPromise)
        return datasetPromise;
    datasetPromise = fetch(DATA_URL, { cache: "force-cache" })
        .then(async (response) => {
        if (!response.ok)
            throw new Error(`場所データを読み込めませんでした (${response.status})`);
        const value = await response.json();
        if (!isLocalPlaceDataset(value))
            throw new Error("場所データの形式が不正です");
        return value;
    })
        .catch((error) => {
        datasetPromise = null;
...
function textScore(candidate, query) {
    if (!candidate || !query)
        return null;
    if (candidate === query)
        return 1000;
    if (candidate.startsWith(query))
        return 780 - Math.min(80, candidate.length - query.length);
    const at = candidate.indexOf(query);
    if (at >= 0)
        return 610 - Math.min(100, at * 4);
    if (query.startsWith(candidate) && candidate.length >= 2)
        return 500;
    ...
    if (query.length >= 3 && candidate.length <= 32) {
        ...
        if (distance <= max)
            return 430 - distance * 80;
    }
    return null;
}
function scorePlace(place, categoryLabel, terms) {
    ...
    for (const term of terms) {
        ...
        const best = Math.max(nameScore ?? -1, aliasScore !== null ? aliasScore - 25 : -1, areaScore !== null ? areaScore - 300 : -1, categoryScore !== null ? categoryScore - 340 : -1);
        if (best < 0)
            return null;
        total += best;
    }
...
export function searchLocalPlaces(dataset, query, limit = 8) {
    const terms = queryTerms(query);
    if (!terms.length || normalizePlaceText(query).length < MIN_QUERY_LENGTH)
        return [];

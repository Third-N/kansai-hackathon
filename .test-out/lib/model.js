...
export function buildSegments(plan, spots, startMin, travelTable) {
    let t = startMin;
    const out = [];
    plan.forEach((item, i) => {
        if (i > 0) {
...
export function simulate(plan, spots, startMin, opts = {}) {
    ...
    const timeline = raw.map((g) => {
        ...
        if (g.type === "move") {
            ...
            if (g.startMin > s.closeMin - d)
                lateArrivals.push({ spotId: s.id, arriveMin: g.startMin });
            continuousMoveMin = 0;
            if (s.kind === "meal") {
                if (d >= 15) {
                    hp += K.mealHpRecovery;
        ...
        if (hp <= 0 && collapseMin === null)
            collapseMin = g.startMin + d / 2;
        if (hp < BREAKDOWN_HP && lowHpMin === null)
            lowHpMin = g.endMin;
        if (mp < BREAKDOWN_MP && lowMpMin === null)
            lowMpMin = g.endMin;
...
export function crowdAt(spot, min, dayType = "weekday") {
...
export function stateAt(timeline, min) {
    let hp = INITIAL_HP;
    let mp = INITIAL_MP;
 ...
 * 変更が少ない案ほど高く評価する。
 */
export function proposeRevisions(plan, spots, startMin, opts) {
        ...
        for (const order of orders) {
            for (const stayScale of [1, 0.85, 0.7]) {
                ...
                for (const restId of rests) {
                    if (scaled.some((i) => i.spotId === restId))
                        continue;
                    const rest = { spotId: restId, stayMin: 20 };
    ...
    for (const c of candidates) {
        ...
        if (result.collapseMin !== null)
            continue;
        if (result.lateArrivals.length > 0)
            continue;
        if (result.lowHpMin !== null || result.lowMpMin !== null)
            continue;
        // ぎりぎりの案は勧めない。予測が外れる余地を残す
        if (result.endHp < MIN_END_HP)
            continue;
        const trough = Math.min(...result.timeline.map((g) => g.hpOut));
        if (trough < MIN_TROUGH_HP)
            continue;
        let moves = 0;
        c.plan.forEach((item, i) => {
            const orig = originalIndex.get(item.spotId);
            if (orig !== undefined && orig !== i)
                moves += 1;
        });
        const changeCost = moves * 3 +
...
/** 最善の1案だけ要るとき */
export function proposeRevision(plan, spots, startMin, opts) {
    return proposeRevisions(plan, spots, startMin, opts)[0] ?? null;
}
function findMovedLater(before, after) {
    let moved = null;
    let maxShift = 0;

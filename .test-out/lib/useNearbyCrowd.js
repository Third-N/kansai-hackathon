...
export function useNearbyCrowd(spotId) {
    ...
    useEffect(() => {
        ...
        const tick = async () => {
            ...
            const pinged = await sb.rpc("ping_spot", { p_spot_id: spotId, p_member_id: id });
            if (pinged.error)
                return; // オフライン等。今の値のまま次を待つ
            const res = await sb.rpc("nearby_crowd", { p_spot_id: spotId });

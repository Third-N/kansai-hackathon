...
const expired = (t) => !!t.expiresAt && new Date(t.expiresAt).getTime() <= Date.now();
/** 寿命の切れた部屋を閉じる。閉じるとあいことばが解放される。
    Supabase 側の close_expired_rooms() と同じ役割 */
function closeExpired(trips) {
    let changed = false;
...
function readRounds() {
    try {
        return JSON.parse(storage().getItem(RKEY) ?? "[]");
    }
    catch {
        return [];
    }
}
function writeRounds(rs) { storage().setItem(RKEY, JSON.stringify(rs)); }
/* 票は本来クライアントから読めてはいけない。
   Supabase では vetoes を SELECT 不可の RLS にして、集計は RPC の中だけで行う。
   ローカル実装では同じ約束を守るため、この2つの関数の外に票を出さない。 */
function readSubmissions() {
    try {
        return JSON.parse(storage().getItem(SKEY) ?? "[]");
    }
    catch {
        return [];
    }
}
function writeSubmissions(ss) { storage().setItem(SKEY, JSON.stringify(ss)); }
function readVetoes() {
    try {
        return JSON.parse(storage().getItem(VKEY) ?? "[]");
    }
    catch {
        return [];
    }
}
function writeVetoes(vs) { storage().setItem(VKEY, JSON.stringify(vs)); }
export const localStore = {
    ...
    async getActiveTrip() {
        const me = myMemberId();
        return (closeExpired(read()).find((t) => t.status === "running" &&
            t.date === isoDate() &&
            t.members.some((m) => m.id === me)) ?? null);
    },
    async getTrip(id) {
    ...
    async joinByCode(c, label) {
        const trips = closeExpired(read());
        const i = trips.findIndex((t) => t.code === c && t.mode === "party" && t.status !== "done" && !expired(t));
        if (i < 0)
            return null;
        // すでに入っている端末。開き直しただけなので名前は触らない
        const me = myMemberId();
        if (trips[i].members.some((m) => m.id === me))
            return trips[i];
        if (trips[i].locked)
            throw new Error("この待合はもう閉じています");
        if (trips[i].members.length >= ROOM_CAPACITY)
            throw new Error("この待合はいっぱいです");
        const member = {
            id: me,
    ...
    async castVetoes(roundId, memberId, optionIds) {
        ...
        if (i < 0 || rounds[i].status === "revealed")
            return;
    ...
    async reveal(roundId) {
        ...
        // 冪等。既に開示済みならそのまま返す
        if (rounds[i].status === "revealed")
            return rounds[i];
        // 開示時刻より前なら何もしない
        if (new Date(rounds[i].revealAt).getTime() > Date.now())
            return rounds[i];

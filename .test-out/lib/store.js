...
export const storeKind = configured ? "supabase" : "local";
const remote = configured
    ? deferredStore(async () => {
        ...
        if (!client)
            throw new Error("Supabase の設定を読み込めませんでした");
 ...
 */
export const store = routedStore(() => demo.get().enabled || !remote ? localStore : remote);

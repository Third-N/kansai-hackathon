...
test("推奨滞在時間ぶん居ないと、気分の『満喫』ボーナスが付かない", () => {
    const spot = makeCustomSpot({ name: "テスト", category: "temple", priority: 1100, ...KYOTO });
    const spots = { [spot.id]: spot };
    const shortStay = [{ spotId: spot.id, stayMin: 5 }];
    const fullStay = [{ spotId: spot.id, stayMin: spot.recommendedStayMin }];
    const short = simulate(shortStay, spots, 600);
    const full = simulate(fullStay, spots, 600);
    assert.ok(full.endMp > short.endMp, `じっくり滞在(${full.endMp})が素通り(${short.endMp})より気分が上がっていない`);
});
